...
| Student auth            | Supabase Auth. Guest-first stays the default (ADR 0005 supersedes ADR 0003) — an account is an _opt-in_ upgrade that exists only to sync saved opportunities across devices, never a requirement to browse or save. **19 Aug reversal (Elena's call, partially supersedes ADR 0005):** submitting an opportunity via `/submit` now _does_ require a signed-in account — the "never a requirement to... submit" clause no longer holds for that one flow. `/submit` redirects signed-out visitors to `/sign-in?next=/submit`; the RLS insert policy was narrowed from `anon, authenticated` to `authenticated` only in `supabase/migrations/20260819130000_require_auth_for_lead_submission.sql`. Email/password (verified email required) or OAuth (Microsoft, Apple; Google stays behind a flag until `docs/legal/` is reviewed and live). Self-declared 16+, no parental-consent flow, `profiles.role` (`ambassador`/`moderator`/null) is never self-assignable through any signup path. |
...
- **Shell** — landing, `/about`, empty states, `/submit`, share cards. Full collage: halftone, cut-paper shapes, tape, saturated blocks, rotated stickers. `/submit` is a "organising something selective, or know someone who is?" contact-style form — a signed-in student account is required (19 Aug reversal, see the student-auth row in §3), but it's still not the authenticated _ambassador_ tool at `/admin`. It writes a `lead` row into the same review queue ambassadors work from in `/admin`; it does not let the submitter set `review_state` or bypass the publish gate. Built — see §12 step 10.
...
10. **`/submit`** — "organising something selective, or know someone who is?" intake form, feeding the same review queue ambassadors use in `/admin`. **Built**, and gated behind a signed-in student account since 19 Aug (see §7 for framing, and the student-auth row in §3 for the reversal). Nav-wired (top nav, tablet menu, footer) as of the same date.
...
- **Resolved 19-20 Aug:** the `packages/taxonomy` typecheck hang turned out to be two stacked problems, not one. First, `packages/taxonomy`, `packages/design`, and `packages/contracts` were all genuinely orphaned (confirmed nothing in `src/` imported any of them) and got deleted outright, along with the rest of the dead Payload/Expo/Python/GitHub-issue pipeline (`scripts/*.py`, `opportunities/` markdown tree, `Dockerfile.cms`, `docker-compose.yml`, `render.yaml`, `AGENTS.md`, `.codex/`). Second, and separately: the repo's working copy lived under `~/Documents`, which macOS was actively iCloud-syncing — `fileproviderd` was starving every `tsc`/`next build` of CPU (20-50 minutes for what should take seconds, sometimes never finishing). Moved the whole repo to `~/Developer/ChatGPT/OffMap` (outside iCloud's sync scope) — `typecheck` now runs in ~3-5s. If a future session finds a repeat of this symptom, check `~/Developer/ChatGPT/OffMap` still isn't under an iCloud/Dropbox-managed folder before assuming it's a code problem.
...
- Repo relocated from `~/Documents/ChatGPT/OffMap` to `~/Developer/ChatGPT/OffMap` (see the resolved-19-20-Aug note above) — point any terminal/IDE session at the new path.
- `/submit` reversal (Elena's call): now requires a signed-in account rather than being guest-accessible — see the student-auth row in §3 and §7/§12 step 10. Wired into primary nav (top nav, tablet menu, footer) for the first time; it previously had no link anywhere in the UI.
- Fixed a production 500 across most of the `(public)` route group: `legal-doc.tsx` read `docs/legal/*.md` via a variable path, which defeated Next's file tracer — the `.md` files silently didn't ship to Vercel. Rewritten so each page reads its own file with a literal path.
- Sign-up silently "succeeding" with no email sent for an already-registered address (Supabase's anti-enumeration behavior) is now caught and redirects to `/sign-in` with a clear notice. Added `/forgot-password` and `/reset-password`.
- **Profile personalization UI is built** on top of the schema from `20260819172931_profile_personalization.sql` (which landed with no UI yet): sign-up collects first/last name, status, and country; a new `/onboarding` 4-step walkthrough (fields → goals → experience level → a live recommended-opportunities sweep) runs once per account, reached from both `/callback` and password sign-in whenever `onboarding_completed_at` is still null; `SaveButton` gained a three-way notify bell (applications open / start writing / closing soon); `/profile` is a real page now — the onboarding-collected fields plus a Goals/Apply/Applied/Archived board of saved opportunities. Verified end-to-end against the live database with a real signed-up test account, then fully deleted.
- Uncommitted, in-progress on top of this (not mine, left alone): Vercel Analytics + Speed Insights wiring in `package.json`/`src/app/layout.tsx`, from a session that appears to have disconnected mid-work.
