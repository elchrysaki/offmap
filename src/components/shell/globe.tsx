...
// OffMap defaults: transparent ocean (no lit sphere — flat/no-gradient per
// the design system), ink dot mesh, paper outline, marigold markers on the
// network's cities.
export default function Globe({
  ...
  useEffect(() => {
    ...
    // Great-circle-ish arcs connecting network cities — a quadratic bezier
    // from surface point to surface point, bowed outward from the globe
    // centre so it reads as a route rather than a chord cutting through it.
    let arcMeshes: Mesh[] = [];
    const updateArcs = () => {
      arcMeshes.forEach((mesh) => globeGroup.remove(mesh));
      arcMeshes = [];
      arcs.forEach((arc) => {
        if (!arc?.from || !arc?.to) return;
        const p1 = latLngToPosition(arc.from.lat, arc.from.lng);
        const p2 = latLngToPosition(arc.to.lat, arc.to.lng);
        const v1 = new Vector3(p1.x, p1.y, p1.z).multiplyScalar(globeRadius);
        const v2 = new Vector3(p2.x, p2.y, p2.z).multiplyScalar(globeRadius);
        const chord = v1.distanceTo(v2);
        const mid = v1.clone().add(v2).multiplyScalar(0.5);
        const dir = mid.lengthSq() > 1e-6 ? mid.clone().normalize() : new Vector3(0, 1, 0);
        const control = dir.multiplyScalar(mid.length() + chord * arc.elevate);

        const segments = 40;
        const points: Vector3[] = [];
        for (let i = 0; i <= segments; i++) {
          const t = i / segments;
          const a = v1.clone().multiplyScalar((1 - t) * (1 - t));
          const b = control.clone().multiplyScalar(2 * (1 - t) * t);
          const c = v2.clone().multiplyScalar(t * t);
          points.push(a.add(b).add(c));
        }
        const curve = new CatmullRomCurve3(points);
        const arcMaterial = new MeshBasicMaterial({ color: new Color(arc.color) });
        const tubeGeometry = new TubeGeometry(curve, segments, (arc.width / 10) * 0.01, 6, false);
        const tubeMesh = new Mesh(tubeGeometry, arcMaterial);
        globeGroup.add(tubeMesh);
        arcMeshes.push(tubeMesh);
      });
    };
  ...
  if (error) {
    // Fails silently into nothing visible — this is a decorative corner
    // accent, not content; a visible error box would be worse than absence.
    return <div style={containerStyle} aria-hidden="true" />;
  }

  return <div ref={containerRef} style={containerStyle} aria-hidden="true" />;
}
